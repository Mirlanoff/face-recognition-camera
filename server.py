...
Analytics
GET  /api/analytics/emotions_pie       -> emotion distribution (filter: lesson_id | class_id | all)
GET  /api/analytics/emotions_timeline  -> emotion counts bucketed by minute for a lesson
GET  /api/analytics/attendance_by_class -> attendance % per class (last 30 days)
GET  /api/analytics/student_summary    -> per-student emotion stats
...
def analytics_emotions_pie(
  ...
  """Emotion distribution. Filter by lesson_id, class_id, or all (no filter)."""
...
def analytics_emotions_timeline(
  ...
  """For a single lesson: emotion counts bucketed by minute since lesson start."""
  lesson = db.query(Lesson).filter(Lesson.id == lesson_id).first()
  if lesson is None:
    raise HTTPException(status_code=404, detail="Сабак табылган жок")
...
def analytics_attendance_by_class(
  ...
  """For each class: how many students on average attended each lesson in the last N days."""
...
def analytics_student_summary(
  ...
  """Per-student: total lessons seen, dominant emotion, negative emotion ratio."""
  q = db.query(Student)
  if class_id is not None:
    q = q.filter(Student.class_id == class_id)
