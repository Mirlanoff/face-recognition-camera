...
Hardware-accelerated h264 decode via ffmpeg subprocess.

This version:
- Tries multiple ffmpeg hwaccel backends in order: d3d11va -> dxva2 -> software
- Prints ffmpeg stderr so we can see real errors (auth, network, codec)
...
# Order to try ffmpeg hwaccel backends. None = software (last resort).
...
def _stderr_pump(stream, label):
    """Print ffmpeg stderr so we can see real errors."""
    try:
...
def probe_ffmpeg_pipeline(ffmpeg_path, url, width, height, hwaccel, frame_size):
    """
    Try to launch ffmpeg with a given hwaccel and read ONE full frame within timeout.
    Returns the Popen object on success (with stderr already being pumped), else None.
    """
    ...
    # Pump stderr in background so we can see ffmpeg's complaints
    ...
    while time.time() < deadline:
        ...
        if len(buf) >= frame_size:
            ...
            # Hand the proc back; reader thread will continue from here.
            # But we already consumed one frame's worth of stdout — that's ok,
            # the reader loop reads frame-by-frame anyway.
            return proc
...
class FFmpegCameraReader(threading.Thread):
    ...
    def start_pipeline(self):
        """Try hwaccel backends in order; keep the first that delivers a frame."""
        ...
        while self.running:
            if self.process is None or self.process.poll() is not None:
                if self.process is not None:
                    print("[!] ffmpeg exited (code " + str(self.process.returncode) + "), restarting...")
                time.sleep(0.5)
...
def main():
    ...
    # Start the pipeline IN THE MAIN THREAD so we see hwaccel probing output cleanly
    if not camera.start_pipeline():
        print("[X] Could not start ffmpeg with any hwaccel backend.")
        print("    Check the [ffmpeg/...] lines above for the real error")
        print("    (often: wrong RTSP password, camera offline, firewall).")
        return

    camera.start()

    # Wait for the reader thread to populate the first decoded frame
    t0 = time.time()
    while camera.read() is None:
        if time.time() - t0 > CONNECT_TIMEOUT_SECONDS:
            print("[X] Pipeline started but no decoded frames arrived. Exiting.")
            camera.stop()
            return
        time.sleep(0.1)
    ...
    try:
        while True:
            ...
            cv2.putText(
                ...
                "  HW: " + (camera.hwaccel if camera.hwaccel else "sw"),
