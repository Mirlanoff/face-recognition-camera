...
# Connection timeout in seconds when opening RTSP
...
"""Open RTSP stream over TCP with a small buffer to avoid lag/timeouts."""
# Force TCP transport + short stimeout (microseconds). Must be set BEFORE VideoCapture.
os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = (
    f"rtsp_transport;tcp|stimeout;{CONNECT_TIMEOUT_SECONDS * 1_000_000}"
)
...
print(f"[+] Connecting to camera (TCP): {RTSP_URL}")
...
# Verify we can actually read a frame (TCP can connect but stream may be empty)
ret, _ = cap.read()
if not ret:
...
while True:
    ...
    if not ret:
        ...
        print(f"[!] Failed to grab frame ({consecutive_failures}). Reconnecting...")
        cap.release()
        time.sleep(1)
        cap = open_rtsp(RTSP_URL)
