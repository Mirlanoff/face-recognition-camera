...
- If camera is offline, stays idle and waits — does NOT fabricate fake data
- Runs InsightFace detection + recognition every N frames on REAL camera frames
- Matches embeddings against students of that lesson's class
- Runs FERPlus emotion classifier ONLY on faces actually detected in the live frame
- Writes Attendance / EmotionLog / Alert rows to SQLite
- Pushes JSON events to an async.Queue consumed by the WebSocket layer
"""
...
# ---------- Config ----------
# Password Akniet12@ — '@' MUST be url-encoded as %40 inside the RTSP URL,
# otherwise the parser treats it as a host separator and connection fails.
...
RECONNECT_DELAY_S = 5.0  # wait between reconnect attempts when camera drops
...
def __init__(self, lesson_id, event_queue):
    ...
    # Track which students are currently visible (have been seen recently)
    self.currently_visible = set()
    self.VISIBLE_TIMEOUT_S = 3.0  # mark as "left frame" after this
...
def run(self):
    ...
    cap = open_camera()
    if cap is None:
        print("[worker " + str(self.lesson_id) + "] CAMERA OFFLINE — entering wait-and-retry loop")
        self.push_event({
            "type": "worker_started",
            "lesson_id": self.lesson_id,
        ...
        self.push_event({
            "type": "worker_error",
            "lesson_id": self.lesson_id,
            "message": "Камера офлайн. Текшерип жатам... (RTSP жеткиликсиз)",
        })
        # Wait-and-retry — do NOT fabricate fake emotions
        while not self.stop_flag.is_set():
            ...
            cap = open_camera()
            if cap is not None:
                print("[worker " + str(self.lesson_id) + "] camera came online, switching to live")
                self.push_event({
                    "type": "worker_started",
                    "lesson_id": self.lesson_id,
                    "roster_size": len(roster),
...
def run_live(self, cap, roster):
    """
    Process REAL camera frames. Only fires events for faces actually
    detected in the current frame — never fabricates emotions from
    registration photos.
    """
    frame_idx = 0
    ...
    while not self.stop_flag.is_set():
        ...
        if not ok or frame is None:
            ...
            if consecutive_failures > 100:
                print("[worker] too many read failures — attempting reconnect")
                cap.release()
                cap = open_camera()
                if cap is None:
                    self.push_event({
            ...
            if do_emotion:
                try:
                    ...
                    if x2 > x1 and y2 > y1:
                        ...
                        # Only log if it CHANGED — suppresses event spam
