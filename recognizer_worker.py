...
# ------------ Live window config ------------
# Default OFF: view_camera.py is the user-facing display window.
# Worker stays headless to avoid duplicate windows / double RTSP connections.
# Set env SHOW_WINDOW=1 ONLY for standalone debug runs without view_camera.py.
...
def __init__(self, lesson_id, event_queue):
    ...
    self.paused = False
    self.last_overlay = {}   # student_id -> (bbox, name, emotion, ts) for drawing between recogn frames
...
def run(self):
    ...
    if cap is None:
        ...
        self.push_event({
            "type": "worker_error",
            "lesson_id": self.lesson_id,
            "message": "Камера офлайн. Техерипи ждем...",
        })
            ...
            if consecutive_failures > 100:
                ...
                if cap is None:
                    self.push_event({
                        "type": "worker_error",
                        "lesson_id": self.lesson_id,
                        "message": "Камера менен байланыш ұзiлдi",
                    })
                    return
                consecutive_failures = 0
            time.sleep(0.05)
        ...
        if not keep_going:
            ...
            if cap is None:
                self.push_event({
                    "type": "worker_error",
                    "lesson_id": self.lesson_id,
                    "message": "Кайра туташку ишке ашкан жок",
                })
                return
